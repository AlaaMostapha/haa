    ...
    <div class="container user-wrapper">
        <div class="sign-inner col-md-8 col-xs-12">
            ...
            <div class="sign-form col-xs-12">
                @include('frontend.user.profile.form', ['signupPage' => true, 'submitUrl' => route('user.register'), 'submitButtonText' => __('Signup')])
            </div>
            <div class="choose-inner col-xs-12">
                <p>{{ __('Already have an account ?') }} <a href="{{ route('user.login') }}">{{ __('Login') }}</a></p>
            </div>
        </div>
    </div>
</div>

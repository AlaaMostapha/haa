                        ...
                        <table class="rwd-table w-100" id="data-table-th">
                            ...
                            <tbody>
                                @if ($tasks->total() > 0)
                                    ...
                                    <tr>
                                        <td data-th="{{ __('companytask.title') }}" class="data">
                                            @if($task->company->company_logo)
                                            <div class="t-img mb-1">
                                                <a href="{{route('company.company.detail' ,[optional($task->company)->id])}}">
                                                    <img src="{{ $task->company->company_logo }}" alt="{{ $task->company->name }}">
                                                </a>    
                                            </div>
                                            <h4 class="font-bold">{{optional($task->company)->name}}</h4>
                                            @else
                                            <div class="t-img mb-1">
                                                <a href="{{route('company.company.detail' ,[optional($task->company)->id])}}">
                                                <img src="{{asset('/frontend/images/company-default.jfif')}}" alt="{{ $task->company->name }}">
                                                </a>
                                            </div>
                                            <h4 class="font-bold text-center">{{optional($task->company)->name}}</h4>
                                            @endif
                                        </td>
                                        ...
                                        <td data-th="{{ __('companytask.Duration') }}">
                                            <div class="app-n text-center">
                                                <a href="{{ route('user.task.detail', ['companyTask' => $task]) }}">{{ __('View') }}</a>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach

...
@foreach ($formComponents as $formComponentName => $formComponent)
    ...
    <div class="form-group{{ $errors->has($formComponent['accessor']) ? ' has-error' : '' }}">
        ...
        <label for="{{ $formComponentName }}" class="col-sm-2 control-label">{{ __('MentorEvaluation.' . $formComponent['accessor'], [], 'ar') }}@if (isset($formComponent['attr']) && isset($formComponent['attr']['required']) && $formComponent['attr']['required']) *@endif</label>
        @else
        ...
        <div class="col-sm-10">
            ...
            @if (isset($formComponent['type']) && $formComponent['type'] == 'link')
                <a href="{{ env('UPLOAD_URL') . $formComponent['path'] }}" > {{ $formComponent['label'] }} </a>
            @endif
            ...
            @if (isset($formComponent['type']) && $formComponent['type'] == 'image')
                ...
                >
                @if(old($formComponentName))
                    <img src="{{ env('UPLOAD_URL').old($formComponentName) }}" height="50px" />
                @endif
            @endif
        ...
        @endif
            ...
            @foreach($formComponent['options'] as $selectOption)
                ...
                <label><input type="radio" @if(isset($formComponent['attr']) && isset($formComponent['attr']['required']) && $formComponent['attr']['required']) required="" @endif value="{{ $selectOption['value'] }}" name="{{ $formComponentName }}" @if(old($formComponentName) == $selectOption['value']) checked="" @endif> {{ $selectOption['label'] }}</label>&nbsp;&nbsp;&nbsp;
    ...
    @elseif($formComponent['type'] == 'section')
        <h4> {{ __('MentorEvaluation.' . $formComponentName, [], 'ar') }} </h4>
        <hr>
    @else

...
<!-- -->
<div>
    <h3 style="color: #18a689 ;font-weight:500; display:inline; margin-left:20px"> {{__("user.total_of_student")}}  </h3> <span style="color:black;font-weight:700;">{{$majorUsers->total()}}</span>  
    </div>
    @if(count($majorUsers) > 0)
    <div class="row">
        <div class="col-lg-12">
                <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($majorUsers as $majorUser)
                                    <tr>
                                        <td>{{optional($majorUser)->firstName }}</td>
                                        <td>{{optional($majorUser)->lastName }}</td>
                                        <td>{{optional($majorUser)->email }}</td>
                                        <td> 
                                        <a href="{{route('dashboard.user.show' ,optional($majorUser)->id)}}" class="btn btn-white btn-sm"><i class="fa fa-eye"></i> عرض</a>
